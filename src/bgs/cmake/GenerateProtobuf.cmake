function(protobuf_generate)
  ...
  if(NOT protobuf_generate_GENERATE_EXTENSIONS)
    ...
    elseif(protobuf_generate_LANGUAGE STREQUAL bgs)
      set(protobuf_generate_GENERATE_EXTENSIONS .pb.h .pb.cc)
...
function(PROTOBUF_GENERATE_BGS SRCS HDRS)
  cmake_parse_arguments(protobuf_generate_bgs "" "EXPORT_MACRO;DESCRIPTORS" "" ${ARGN})

  set(_proto_files "${protobuf_generate_bgs_UNPARSED_ARGUMENTS}")
  if(NOT _proto_files)
    message(SEND_ERROR "Error: PROTOBUF_GENERATE_BGS() called without any proto files")
    return()
  endif()

  if(PROTOBUF_GENERATE_BGS_APPEND_PATH)
    set(_append_arg APPEND_PATH)
  endif()

  if(protobuf_generate_bgs_DESCRIPTORS)
    set(_descriptors DESCRIPTORS)
  endif()
  ...
  set(_outvar)
  protobuf_generate(${_append_arg} ${_descriptors}
    LANGUAGE bgs
    EXPORT_MACRO ${protobuf_generate_cpp_EXPORT_MACRO}
    OUT_VAR _outvar ${_import_arg}
    PLUGIN "protoc-gen-bgs=$<TARGET_FILE:protoc-gen-bgs>"
    PROTOS ${_proto_files})

  set(${SRCS})
  set(${HDRS})
  if(protobuf_generate_bgs_DESCRIPTORS)
    set(${protobuf_generate_bgs_DESCRIPTORS})
  endif()
  ...
  foreach(_file ${_outvar})
    if(_file MATCHES "cc$")
      list(APPEND ${SRCS} ${_file})
    elseif(_file MATCHES "desc$")
      list(APPEND ${protobuf_generate_bgs_DESCRIPTORS} ${_file})
    else()
      list(APPEND ${HDRS} ${_file})
    endif()
  endforeach()
  ...
  set(${HDRS} ${${HDRS}} PARENT_SCOPE)
  if(protobuf_generate_bgs_DESCRIPTORS)
    set(${protobuf_generate_bgs_DESCRIPTORS} "${${protobuf_generate_bgs_DESCRIPTORS}}" PARENT_SCOPE)
  endif()
endfunction()
